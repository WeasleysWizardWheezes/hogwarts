...
public class VehicleService {
    ...
    public Vehicle createVehicle(VehicleRequest request) {
        VehicleGroup vehicleGroup = vehicleGroupRepository.findById(
                        request.getVehicleGroupId())
                .orElseThrow(() -> new VehicleGroupNotFoundException(
                        request.getVehicleGroupId()));

        if (request.getBaujahr() != null && (request.getBaujahr() < 1900
                || request.getBaujahr() > Year.now().getValue())) {
            throw new IllegalArgumentException("Baujahr muss zwischen 1900 und "
                    + Year.now().getValue() + " liegen");
        }
    ...
    public List<Vehicle> getAllVehicles(VehicleStatus status, UUID vehicleGroupId,
        ...
        if (includeArchived) {
            if (status != null && vehicleGroupId != null) {
                ...
                return vehicleRepository.findByVehicleGroupIdAndIsArchived(
                        vehicleGroupId, true);
            } else {
            ...
            if (status != null && vehicleGroupId != null) {
                ...
                return vehicleRepository.findByVehicleGroupIdAndIsArchivedFalse(
                        vehicleGroupId);
            } else {
