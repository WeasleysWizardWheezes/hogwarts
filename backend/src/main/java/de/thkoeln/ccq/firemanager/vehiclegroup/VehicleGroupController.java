...
import de.thkoeln.ccq.firemanager.vehiclegroup.dto.UpdateVehicleGroupRequest;
...
public class VehicleGroupController {
    ...
    @PutMapping("/{vehicleGroupId}")
    public ResponseEntity<VehicleGroup> updateVehicleGroup(
            @PathVariable UUID vehicleGroupId,
            @Valid @RequestBody UpdateVehicleGroupRequest request
    ) {
        var vehicleGroup = vehicleGroupService.update(vehicleGroupId, request.name(), request.description());
        return ResponseEntity.ok(vehicleGroup);
    }
