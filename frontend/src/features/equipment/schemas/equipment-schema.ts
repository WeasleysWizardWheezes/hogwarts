...
export const createDeviceSchema = v.object({
  name: v.pipe(v.string().min(1, "Gerätename ist erforderlich")),
  serialNumber: v.pipe(v.string().min(1, "Seriennummer ist erforderlich")),
  type: v.pipe(v.string().min(1, "Typ ist erforderlich")),
  location: v.pipe(v.string().min(1, "Lagerort ist erforderlich")),
})
